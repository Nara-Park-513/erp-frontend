...
const SHIPMENT_STATUS_OPTIONS = ["출고대기", "출고완료", "배송중", "완료"];
...
export default function ShipmentProcessModal({
  show,
  id,
  onHide,
  onChanged,
  ...
  const [progressText, setProgressText] = useState("출고대기");
  ...
  const normalizeDetail = (r: any): ShipmentDetail => ({
    id: Number(r?.id ?? r?.orderId ?? 0),
    orderNo: String(r?.orderNo ?? r?.orderCode ?? r?.no ?? ""),
    orderName: String(r?.orderName ?? r?.name ?? ""),
    progressText: String(
      r?.progressText ?? r?.progress ?? r?.stepName ?? r?.statusText ?? r?.status ?? "출고대기"
    ),
  });
  ...
  const loadDetail = async (targetId: number) => {
    ...
    try {
      ...
      setProgressText(d.progressText || "출고대기");
      setShipmentMemo("");
    } catch (e: any) {
      console.error("출고 상세 조회 실패", e);
      openAlertModal("error", "상세 조회 실패", "출고 상세 조회에 실패했습니다. 콘솔을 확인해 주세요.");
      setDetail(null);
    } finally {
      setLoading(false);
    }
  ...
  const validate = () => {
    ...
    if (!progressText.trim()) return "출고상태를 선택하세요.";
    return "";
  };

  const handleSave = async () => {
    if (id == null) {
      openAlertModal("warning", "처리 불가", "출고 대상 정보가 없습니다.");
      return;
    }
    ...
    try {
      ...
      openAlertModal("success", "처리 완료", "출고 상태가 저장되었습니다.");
      await Promise.resolve(onChanged());
    } catch (e: any) {
      console.error("출고 저장 실패", e);
      openAlertModal("error", "저장 실패", "출고 상태 저장에 실패했습니다. 콘솔을 확인해 주세요.");
    } finally {
      setSaving(false);
    }
  };
